...
The static, steady-state temperature of the bolometer ($T_("bol")$) rests slightly higher than the bath temperature ($T_s$) due to the continuous Joule power dissipation ($P = V_("bol") I_("bol")$) from the readout circuit, defined as $T_("bol") = T_s + P/G$. This power dissipation introduces a phenomenon known as electrothermal feedback: if the crystal heats up, the NTD resistance drops, which alters the dissipated power and consequently affects the temperature. When characterizing a detector, $V-I$ load curves are measured across different bath temperatures. As $I_("bol")$ increases, the load curve eventually bends into a region of negative dynamic resistance due to this feedback. The optimal working point is carefully selected just below this inversion threshold to maximize the signal-to-noise ratio while ensuring thermal stability.
