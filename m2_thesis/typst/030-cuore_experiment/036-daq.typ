...
The `DataReader` is the primary process running on the reader computers. It receives the incoming data and writes it to a shared memory block structured as a circular buffer. Strict pointer management allows concurrent access by the Event Builder and auxiliary diagnostic processes—such as the online oscilloscope—ensuring data is read in a precise chronological sequence without the risk of accessing incomplete fragments. Furthermore, the reader process continuously streams the raw, unfiltered waveforms directly to disk, archiving them as continuous data files (`.RCDF`).
